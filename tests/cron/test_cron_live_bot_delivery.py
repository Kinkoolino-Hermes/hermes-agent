...
@pytest.mark.parametrize("output_file", [None, "digest.md"])
def test_result_records_pending_until_terminal_receipt(tmp_path, monkeypatch, output_file):
    ...
    error = delivery._deliver_result(job, "payload", output_file=output_file)
    assert error is None
    queued = updates[-1]["last_delivery_queued"]
    assert queued and next(iter(queued.values()))["status"] == "queued"
    assert job["last_delivery_queued"] == queued
    assert job.get("_output_file") == output_file
    ...
    assert "_output_file" not in job
