...
class TestConsolidationProposalSurfaces:
    ...
    def review_dispatch(self):
        ...
        try:
            yield {"session_id": "review-session"}
        finally:
    ...
    def test_staged_proposal_surfaces_in_summary(self, tmp_path, monkeypatch, review_dispatch):
        import json
        ...
        try:
            raw = memory_tool(
                ...
                tool_call_id="c1", **review_dispatch)
        finally:
    ...
    def test_near_limit_denial_end_to_end(self, tmp_path, monkeypatch, review_dispatch):
        ...
        try:
            add_raw = memory_tool(
                action="add", content="y" * 600, store=store, tool_call_id="c1", **review_dispatch)
            replace_raw = memory_tool(
                action="replace", old_text="seed entry one", content="merged entry", store=store,
                tool_call_id="c2", **review_dispatch)
        finally:
            reset_current_write_origin(token)
