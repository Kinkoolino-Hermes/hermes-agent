...
class TestBackgroundReviewDeleteGate:
    ...
    @pytest.fixture
    def review_dispatch(self):
        from gateway.session_context import clear_session_vars, set_session_vars
        from tools.write_approval import _owner_only_permissions_supported

        if not _owner_only_permissions_supported():
            pytest.skip("Pending schema v2 requires POSIX owner-only descriptor storage")
        tokens = set_session_vars(source="cli", profile="default")
        try:
            yield {"session_id": "review-session", "tool_call_id": "review-call"}
        finally:
            clear_session_vars(tokens)

    @pytest.mark.linux_only
    def test_remove_staged_not_applied(self, store, tmp_path, monkeypatch, review_dispatch):
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
        store.add("memory", "never create records without permission")
        token = set_current_write_origin("background_review")
        try:
            result = json.loads(memory_tool(
                action="remove", old_text="without permission", store=store, **review_dispatch))
        finally:
    ...
    @pytest.mark.linux_only
    def test_replace_staged_in_background_review(self, store, tmp_path, monkeypatch, review_dispatch):
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    ...
    @pytest.mark.linux_only
    def test_batch_containing_remove_staged_whole_batch(self, store, tmp_path, monkeypatch, review_dispatch):
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
