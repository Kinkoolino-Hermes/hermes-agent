...
class _BackgroundReviewReadMarks:
    """Exact read bytes shared by copied tool contexts within one review run."""
...
def _background_review_has_read(path: Path) -> bool:
    ...
    return marks is not None and marks.contains(_resolved_str(path), content_digest)
