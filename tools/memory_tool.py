...
def _gate_or_stage(
    ...
    *,
    ...
    """Return a staged/blocked result, or ``None`` when the live write may proceed."""
    try:
...
def _apply_write_gate(
    ...
    *,
    ...
    """Gate one mutating op, or a whole batch as a single approval unit."""
    ...
    summary, detail = _STORE_ACTIONS[action][1](label, content, old_text)
    return _gate_or_stage(
        summary,
        detail,
...
def _background_delete_gate(
    action, operations, target="memory", content=None, old_text=None,
    *, session_id=None, tool_call_id=None,
) -> Optional[str]:
...
def memory_tool(action: str = None, target: str = "memory", content: str = None, old_text: str = None,
                ...
                store: Optional[MemoryStore] = None, session_id: Optional[str] = None,
    ...
    if operations:
        ...
        denied = _background_delete_gate(
            action, operations, target, session_id=session_id, tool_call_id=tool_call_id,
        )
        if denied is not None:
            return denied
        # Approval gate: stages (background/gateway) or prompts inline (CLI); off by default.
        gate_result = _apply_write_gate(
            "batch", target, None, None, operations, session_id=session_id,
            tool_call_id=tool_call_id,
        )
    ...
    invalid = (
        _validate_single_op(store, action, target, content, old_text)
        or _background_delete_gate(
            action, None, target, content, old_text,
            session_id=session_id, tool_call_id=tool_call_id,
        )
        or _apply_write_gate(
            action, target, content, old_text, session_id=session_id,
            tool_call_id=tool_call_id,
        )
    )
